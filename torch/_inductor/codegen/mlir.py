...
class MlirVecOverrides(OpOverrides):
    """Map element-wise ops to aten vectorization C++"""

    @staticmethod
    def add(a, b):
        print("Genenrating add2\n")
...
class MlirOverrides(OpOverrides):
...
class MlirKernel(Kernel):
    overrides = MlirOverrides
    sexpr = cexpr
    newvar_prefix = "%"
    suffix = ""
    ...
    def load(self, name: str, index: sympy.Expr):
        ...
        line = f"affine.load %{var}[%{cexpr(index)}] : memref<100xf32>"
    ...
    def store(self, name, index, value, mode=None):
        ...
        if mode is None:
            ...
            line = f"affine.store %{value}, %{var}[%{cexpr(index)}] : memref<100xf32>"
...
class MlirVecKernel(MlirKernel):
    overrides = MlirVecOverrides
...
class MlirTile2DKernel(MlirVecKernel):
    """
    A vector kernel that handles the 2d tiles with the tile size defined in `tiling_factor` on
    the inner-most loop level and one of the outer loop level (`outer_tiling_idx`). When the data
    tile is accessed in a contiguous way from the outer loop axis, a transposition is applied on the
    tile to make the access contiguous from the inner-most loop axis. Then, the same vectorization
    logic from its parent `MlirVecKernel` is leveraged for load/store/compute. The transposed tile load
    ...
    for ...
      for i_outer ...
        for ...
          ...
          for inner_most ... (tail)
            // generated by MlirTile2DTailKernel
            ...
      for i_outer ... (tail)
        for ...
          for ...
            // generated by MlirKernel
            ...
    """
...
class MlirTile2DTailKernel(MlirKernel):
    """
...
class MlirVecKernelChecker(MlirVecKernel):
    def __init__(self, args, num_threads, tiling_factor):
        ...
        self.simd_vec = True
        self.fast_vec_list = []
        for k, v in MlirVecOverrides.__dict__.items():
...
class MlirTile2DKernelChecker(MlirVecKernelChecker):
    """
...
class MlirKernelProxy(MlirKernel):
    ...
    def codegen_nodes(self, nodes):
        ...
        def codegen_kernel(cls, *args):
            with kernel_group.new_kernel(cls, *args) as kernel:
                ...
                # we only count in MlirKernelProxy, not those contained in it
                metrics.generated_kernel_count -= 1
        ...
        scalar_kernel = codegen_kernel(MlirKernel)
        ...
        with torch._inductor.config.patch(inplace_buffers=False):
            ...
            if vec_checker.simd_vec:
                ...
                main_loop.set_kernel(codegen_kernel(MlirVecKernel, tiling_factor))
                ...
                inner_main_loop.set_kernel(
                    codegen_kernel(MlirTile2DKernel, tiling_factor, outer_tiling_idx)
                )
                inner_tail_loop.set_kernel(
                    codegen_kernel(
                        MlirTile2DTailKernel, tiling_factor, outer_tiling_idx
                    )
                )
...
class MlirScheduling:
    ...
    def get_kernel_group(self):
        ...
        if isinstance(V.graph.wrapper_code, CppWrapperCodeGen):
            self.kernel_group = MlirWrapperKernelGroup()
        else:
    ...
    def codegen_nodes(self, nodes):
        ...
        cpp_kernel_proxy = MlirKernelProxy(kernel_group)
...
class MlirKernelArgs:
    ...
    def cpp_argdefs(self):
        ...
        for outer, inner in self.input_buffers.items():
            ...
            arg_defs.append(f"%{inner}: memref<100xf32>")
            call_args.append(self.wrap_ptr_arg(outer, dtype))
            arg_types.append(f"memref<100xf32>")
...
class KernelGroup:
    ...
    def codegen_define_and_call(self, wrapper):
        ...
        code.writelines(["module"])
        with code.indent():
            code.writelines(["" f"func.func @kernel({arg_defs})"])
            with code.indent():
                if enable_kernel_profile:
...
class MlirWrapperKernelGroup(KernelGroup):
    def __init__(self):
        super().__init__()
        self.args = MlirWrapperKernelArgs()
...
class LoopLevel:
    ...
    kernel: MlirKernel = None

    def get_kernels(self) -> List[MlirKernel]:
    ...
    def set_kernel(self, kernel: MlirKernel):
        """
    ...
    def lines(self):
        ...
        if self.parallel:
            ...
            line1 = ""
        line2 = (
            f"affine.for %{self.var}={cexpr(self.offset)} to  {cexpr(self.size)} step 1"
        )
        return [line2]
